...
import { assassinationTwoEntries } from './assassination-classroom-season-2'
import { fateZeroEntries } from './fate-zero'
import { highschoolDeadEntries } from './highschool-of-the-dead'
import { horimiyaEntries } from './horimiya'
import { elfenLiedEntries } from './elfen-lied'
import { kakeguruiEntries } from './kakegurui'
import { slimeSeasonOneEntries } from './slime-season-1'
import { konosubaTwoEntries } from './konosuba-season-2'
import { danmachiEntries } from './danmachi'
import { devilPartTimerEntries } from './devil-is-a-part-timer'
import { anohanaEntries } from './anohana'
import { mobPsychoTwoEntries } from './mob-psycho-100-ii'
import { entertainmentDistrictEntries } from './demon-slayer-entertainment-district'
import { psychoPassEntries } from './psycho-pass'
import { foodWarsEntries } from './food-wars'
import { overlordEntries } from './overlord'
...
export const expansionBanks = [
  ...
  bank('overlord', 'Overlord 第一季', '限定2015年电视动画第一季第 1–13 集官网简介与人物资料，含卡恩村、冒险者活动与夏提雅战剧透；不混入第二季及后续季度、剧场版或原作差异，不补写官网简介未展开的最终制胜细节。', overlordEntries, (reference) => {
    if (typeof reference !== 'number' || !Number.isInteger(reference)) throw new Error('Overlord needs a verified season-one episode or character reference')
    if (reference >= 1 && reference <= 13) return { label: `第一季官网第 ${reference} 集简介`, url: `https://overlord-anime.com/_season1/story.html?st=${reference}` }
    if (reference >= 103 && reference <= 107) return { label: '第一季官网人物资料（图文）', url: `https://overlord-anime.com/_season1/character.html?c=${reference - 100}` }
    throw new Error('Overlord source is outside the verified first-season range')
  }),
  bank('food-wars', '食戟之灵 第一季', '限定2015年电视动画第一季第 1–24 集正版分集简介，含入寮、住宿研修、商店街与秋季选拔预选剧透；不混入第二季及后续季度、OVA或漫画差异，不补写未核验的配方比例和正赛结局。', foodWarsEntries, (reference) => {
    if (typeof reference !== 'number' || !Number.isInteger(reference) || reference < 1 || reference > 24) throw new Error('Food Wars needs a first-season episode from 1 to 24')
    return { label: `万代频道第一季第 ${reference} 集简介`, url: `https://www.b-ch.com/titles/4532/${String(reference).padStart(3, '0')}` }
  }),
  bank('psycho-pass', '心理测量者 第一季', '限定2012–2013年电视动画原版第一季第 1–22 集正版简介及富士电视台世界观资料，含虚拟身份、槙岛追踪与后期调查剧透；不混入新编集版新增场景、第二季及后续作品，不补写未核验的系数阈值、系统内幕与最终结局细节。', psychoPassEntries, (reference) => {
    if (reference === 'terms') return { label: '富士电视台第一季世界观与角色资料', url: 'https://www.fujitv.co.jp/b_hp/psycho-pass/' }
    if (typeof reference !== 'number' || !Number.isInteger(reference) || reference < 1 || reference > 22) throw new Error('Psycho-Pass needs an original first-season episode from 1 to 22')
    return { label: `万代频道原版第一季第 ${reference} 集简介`, url: `https://www.b-ch.com/titles/4106/${String(reference).padStart(3, '0')}` }
  }),
  bank('demon-slayer-entertainment-district', '鬼灭之刃 游郭篇', '限定电视动画游郭篇第 1–11 集官网分集简介，含潜入调查、鬼化失控、合击斩首与中毒危机剧透；不混入刀匠村篇等后续篇章，不补写简介未交代的招式数值、消灭机制或救治步骤。', entertainmentDistrictEntries, (reference) => {
    if (typeof reference !== 'number' || !Number.isInteger(reference) || reference < 1 || reference > 11) throw new Error('Entertainment District needs an episode from 1 to 11')
    return { label: `游郭篇官网第 ${reference} 集简介`, url: `https://kimetsu.com/anime/yukakuhen/story/?id=ep${reference}` }
  }),
  bank('mob-psycho-100-ii', '灵能百分百 第二季', '限定电视动画第二季第 1–13 集及同期官网人物设定，含最上世界、灵幻风波、马拉松与爪决战剧透；不混入OVA或第三季，不补写官网未说明的能力参数及终局处置细节。', mobPsychoTwoEntries, (reference) => {
    if (typeof reference !== 'number' || !Number.isInteger(reference) || reference < 1 || reference > 16) throw new Error('Mob Psycho II needs a season-two episode or character reference')
    if (reference > 13) return { label: '第二季官网人物设定', url: `https://mobpsycho100.com/2nd/chara/${['serizawa', 'shimazaki', 'suzuki'][reference - 14]}.html` }
    return { label: `第二季官网第 ${reference} 集简介`, url: `https://mobpsycho100.com/2nd/story/${String(reference).padStart(2, '0')}.html` }
  }),
  bank('anohana', '未闻花名 电视版', '限定电视动画第 1–11 集及同期官网人物与故事介绍，含伙伴重聚、雪集目击真相、烟花与愿望结果剧透；不混入剧场版，不补写官网简介未交代的最终告别台词和愿望细节。', anohanaEntries, (reference) => {
    if (reference === 'terms') return { label: '电视版官网故事介绍', url: 'https://www.anohana.jp/tv/intro/index.html' }
    if (typeof reference !== 'number' || !Number.isInteger(reference) || reference < 1 || reference > 17) throw new Error('Anohana needs a TV episode or character reference')
    if (reference > 11) return { label: '电视版官网人物设定', url: `https://www.anohana.jp/tv/chara/chara${String(reference - 11).padStart(2, '0')}.html` }
    return { label: `电视版官网第 ${reference} 集简介`, url: `https://www.anohana.jp/tv/story/${reference === 1 ? 'index' : String(reference).padStart(2, '0')}.html` }
  }),
  bank('devil-is-a-part-timer', '打工吧！魔王大人 第一季', '限定2013年第一季第 1–13 集官网简介，含异世界身份、魔力恢复、路西菲尔与沙利叶事件剧透；不混入第二季及后续作品，不补写官网未说明的能力数值，区分梦境、人物猜测与实际行动。', devilPartTimerEntries, (reference) => {
    if (typeof reference !== 'number' || !Number.isInteger(reference) || reference < 1 || reference > 13) throw new Error('Devil Is a Part-Timer needs a first-season episode from 1 to 13')
    return { label: `第一季官网第 ${reference} 集简介（同页倒序排列）`, url: 'https://maousama.jp/1st/story.html' }
  }),
  bank('danmachi', '地错 第一季', '限定电视动画第一季第 1–13 集及同期官网故事介绍，含憧憬一途、莉莉组队、升级与深层救援剧透；不混入OVA、外传或后续季度，不补写未核验的技能数值、等级参数和终局战斗细节。', danmachiEntries, (reference) => {
    if (reference === 'terms') return { label: '第一季官网故事介绍', url: 'https://danmachi.com/danmachi/story/introduction.html' }
    if (typeof reference !== 'number' || !Number.isInteger(reference) || reference < 1 || reference > 13) throw new Error('DanMachi needs a first-season episode from 1 to 13')
    return { label: `第一季官网第 ${reference} 集简介`, url: `https://danmachi.com/danmachi/story/episode${reference}.html` }
  }),
  bank('konosuba-season-2', '为美好的世界献上祝福！ 第二季', '限定电视动画第二季第 1–10 集官网简介，含审判、芸芸登场、巴尼尔与温泉调查剧透；不混入OVA、剧场版或外传，不补写简介未说明的战斗数值、交易金额和最终处置细节。', konosubaTwoEntries, (reference) => {
    if (typeof reference !== 'number' || !Number.isInteger(reference) || reference < 1 || reference > 10) throw new Error('KonoSuba II needs a second-season TV episode from 1 to 10')
    return { label: `第二季官网第 ${reference} 集简介`, url: `https://konosuba.com/2nd/story/?mode=detail&id=${String(reference).padStart(2, '0')}` }
  }),
  bank('slime-season-1', '转生史莱姆 第一季', '限定电视动画第一季第 1–24 集官网简介，含第24集静的前日谭与剧情剧透；不混入闲话回顾篇、OAD或后续季度，不补写简介未说明的技能数值、战斗结果和精灵身份。', slimeSeasonOneEntries, (reference) => {
    if (typeof reference !== 'number' || !Number.isInteger(reference) || reference < 1 || reference > 24) throw new Error('Slime needs a first-season episode from 1 to 24')
    return { label: `第一季官网第 ${reference} 集简介`, url: `https://www.ten-sura.com/anime/tensura/story/no${reference}` }
  }),
  bank('kakegurui', '狂赌之渊 第一季', '限定2017年第一季第 1–12 集官网简介及第一季游戏规则，含人物关系、债务与对局剧透；不混入《××》《双》或真人版，不补写简介未公布的胜负与身份。规则推演题的额外条件以题干为准。', kakeguruiEntries, (reference) => {
    if (reference === 'terms') return { label: '第一季官网游戏规则', url: 'https://kakegurui-anime.com/1st/game_rules/' }
    const episodes = [1000217, 1000221, 1000223, 1000226, 1000227, 1000228, 1000253, 1000254, 1000255, 1000256, 1000257, 1000258]
    if (typeof reference !== 'number' || !Number.isInteger(reference) || reference < 1 || reference > episodes.length) throw new Error('Kakegurui needs a first-season episode from 1 to 12')
    return { label: `第一季官网第 ${reference} 集简介`, url: `https://kakegurui-anime.com/1st/story/detail.php?id=${episodes[reference - 1]}` }
  }),
  bank('elfen-lied', '妖精的旋律 电视版', '限定2004年电视版第 1–13 集公开简介、作品概述与发行方资料，含记忆、亲属关系及追捕剧情剧透；不混入EXTRA特别篇或漫画后续，不补写未公开的能力数值及最终交战结果。', elfenLiedEntries, (reference) => {
    if (reference === 'terms') return { label: 'VAP电视动画与蓝光发行资料', url: 'https://www.vap.co.jp/elfenlied/bd/' }
    if (reference === 'characters') return { label: '正版配信作品概述（目录另含EXTRA）', url: 'https://animestore.docomo.ne.jp/animestore/ci_pc?workId=21868' }
    if (typeof reference !== 'number' || !Number.isInteger(reference) || reference < 1 || reference > 13) throw new Error('Elfen Lied needs a TV episode from 1 to 13')
    return { label: `正版配信电视版第 ${reference} 集简介`, url: `https://animestore.docomo.ne.jp/animestore/ci_pc?workId=21868&partId=21868${String(reference).padStart(3, '0')}` }
  }),
  bank('horimiya', '堀与宫村 2021电视版', '限定2021年电视版第 1–13 集官网简介及第一季人物资料，含告白、关系变化与毕业剧情剧透；不混入《piece》、OVA或真人版，不把传闻、假扮恋人或单方面好感当作已确认的交往。', horimiyaEntries, (reference) => {
    if (reference === 'characters') return { label: '2021电视版官网人物资料', url: 'https://horimiya-anime.com/1st/character/' }
    if (typeof reference !== 'number' || !Number.isInteger(reference) || reference < 1 || reference > 13) throw new Error('Horimiya needs a 2021 TV episode from 1 to 13')
    const id = reference === 1 ? '01' : reference === 13 ? '13' : `ep${String(reference).padStart(2, '0')}`
    return { label: `2021电视版官网第 ${reference} 集简介`, url: `https://horimiya-anime.com/1st/story/?id=${id}` }
  }),
  bank('highschool-of-the-dead', '学园默示录 电视版', '限定2010年电视版第 1–12 集官网简介、作品介绍与人物资料，含人物关系、危机与撤离剧情剧透；不混入OVA或漫画后续，不补写未公开的装备参数、感染机制或最终去向。', highschoolDeadEntries, (reference) => {
    const base = 'https://www.nbcuni.co.jp/rondorobe/anime/hotd/contents/'
    if (reference === 'terms') return { label: 'NBC环球官网作品介绍', url: `${base}hp0003/index00010000.html` }
    const characters = ['小室孝', '宫本丽', '毒岛冴子', '高城沙耶', '平野耕太', '鞠川静香']
    if (typeof reference === 'number' && Number.isInteger(reference) && reference >= 101 && reference <= 106) {
      return { label: `官网人物资料：${characters[reference - 101]}`, url: `${base}hp0006/index${String(108 - reference).padStart(4, '0')}0000.html` }
    }
    const episodes = [16, 18, 19, 20, 25, 27, 28, 29, 30, 31, 32, 33]
    if (typeof reference !== 'number' || !Number.isInteger(reference) || reference < 1 || reference > episodes.length) throw new Error('HOTD needs a TV episode from 1 to 12 or a character from 101 to 106')
    return { label: `官网电视版第 ${reference} 集简介`, url: `${base}hp0005/index${String(episodes[reference - 1]).padStart(4, '0')}0000.html` }
  }),
  bank('fate-zero', 'Fate/Zero 第一季', '限定第一季第 1–13 集官网简介及已公开的基础人物资料，含召唤关系、讨伐安排与王道问答剧透；不混入第14–25集、特典短篇或其他Fate作品，不补写后半部揭晓的身份、动机与结局。', fateZeroEntries, (reference) => {
    if (reference === 'characters') return { label: '官网基础人物资料（页面含全剧人物）', url: 'https://www.fate-zero.jp/characters/index.html' }
    if (typeof reference !== 'number' || !Number.isInteger(reference) || reference < 1 || reference > 13) throw new Error('Fate Zero needs a first-season episode from 1 to 13')
    return { label: `官网第一季第 ${reference} 集简介（同页含后半部）`, url: `https://www.fate-zero.jp/story/#STORY${String(reference).padStart(2, '0')}` }
  }),
  bank('assassination-classroom-season-2', '暗杀教室 第二季', '限定第二季第 1–25 集官网简介，涵盖体育祭、学园祭、班级分歧、宇宙研究及毕业前行动，含重大剧透；不混入第一季、特别篇、剧场版或漫画补充，不补写简介未披露的招式参数、研究概率和未来职业。', assassinationTwoEntries, (reference) => {
    const episodes = [1000726, 1000731, 1000733, 1000754, 1000759, 1000764, 1000770, 1000775, 1000779, 1000784, 1000795, 1000800, 1000803, 1000814, 1000816, 1000828, 1000834, 1000836, 1000838, 1000839, 1000851, 1000854, 1000860, 1000865, 1000876]
    if (typeof reference !== 'number' || !Number.isInteger(reference) || reference < 1 || reference > episodes.length) throw new Error('Assassination Classroom season two needs an episode from 1 to 25')
    return { label: `官网第二季第 ${reference} 集简介`, url: `https://www.ansatsu-anime.com/2014-2016/story/detail.php?id=${episodes[reference - 1]}` }
  }),
  bank('haikyuu-season-2', '排球少年 第二季', '限定第二季第 1–25 集正版发行简介，涵盖东京合宿、训练调整、春高预选与半决赛，含晋级结果剧透；不混入白鸟泽决赛过程、第三季、OAD、剧场版或漫画后续，不补写未披露的逐局比分、招式参数与替补姓名。', haikyuuTwoEntries, (reference) => {
    if (typeof reference !== 'number' || !Number.isInteger(reference) || reference < 1 || reference > 25) throw new Error('Haikyuu season two needs an episode from 1 to 25')
    return { label: `万代频道第二季第 ${reference} 集简介`, url: `https://www.b-ch.com/titles/4898/${String(reference).padStart(3, '0')}` }
  }),
  bank('oregairu-season-1', '我的青春恋爱物语果然有问题 第一季', '限定第一季第 1–12 集及电视番外篇第 13 集官网简介与人物资料，涵盖奉仕部委托、夏令营和文化祭分工，含剧透；不混入OVA、第二季、第三季或小说后续，不补写未披露的关系结论、行动结果与具体方案。', oregairuEntries, (reference) => {
    if (reference === 'characters') return { label: 'TBS第一季人物资料', url: 'https://www.tbs.co.jp/anime/oregairu/1st/chara/' }
    if (typeof reference !== 'number' || !Number.isInteger(reference) || reference < 1 || reference > 13) throw new Error('Oregairu season one needs an episode from 1 to 13')
    return { label: reference === 13 ? 'TBS第一季电视番外篇简介' : `TBS第一季第 ${reference} 集简介`, url: `https://www.tbs.co.jp/anime/oregairu/1st/story/#story${String(reference).padStart(2, '0')}` }
  }),
  bank('fullmetal-alchemist-2003', '钢之炼金术师 2003版', '限定2003年电视版第 1–51 集正版发行简介，含红水研究、人造人身世、但丁与门的重大剧透；不混用FA版、漫画后续、OVA或《香巴拉的征服者》剧场版，不补写未披露的炼成步骤、身份推断与战斗结果。', fma2003Entries, (reference) => {
    if (typeof reference !== 'number' || !Number.isInteger(reference) || reference < 1 || reference > 51) throw new Error('Fullmetal Alchemist 2003 needs an episode from 1 to 51')
    return { label: `万代频道2003版第 ${reference} 集简介`, url: `https://www.b-ch.com/titles/215/${String(reference).padStart(3, '0')}` }
  }),
]
