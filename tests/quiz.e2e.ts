...
for (const [width, height] of [[320, 568], [390, 844], [540, 720], [768, 1024], [1024, 768], [1440, 1000], [844, 390]]) {
  test(`layout and dialogs at ${width}×${height}`, async ({ page }) => {
    ...
    await expect(page.locator('.historical-release')).toHaveCount(28)
    await page.locator('.historical-release summary').first().click()
    await expect(page.locator('.historical-release').first()).toContainText('v0.23.0')
    await expect(page.locator('.historical-release').first()).toContainText('新增《火影忍者 疾风传》风影夺还篇50道配图题')
    await auditLayout(page)
    await page.keyboard.press('Escape')
...
test('discovery filters, sorting, favorites and reset on refresh', async ({ page }) => {
  ...
  for (const [query, id] of [['re0', 'rezero'], ['ONE PIECE', 'one-piece'], ['NARUTO', 'naruto'], ['鬼灭', 'demon-slayer']]) {
    ...
    const matches = quizzes.filter((quiz) => quiz.series === id || quiz.series === 'crossover' || (id === 'naruto' && quiz.series === 'naruto-shippuden'))
  ...
  await expect(page.locator('.quiz-card')).toHaveCount(66)
