...
for (const [width, height] of [[320, 568], [390, 844], [540, 720], [768, 1024], [1024, 768], [1440, 1000], [844, 390]]) {
  test(`layout and dialogs at ${width}×${height}`, async ({ page }) => {
    ...
    await expect(page.locator('.historical-release')).toHaveCount(10)
    await page.locator('.historical-release summary').first().click()
    await expect(page.locator('.historical-release').first()).toContainText('v0.8.1')
    await expect(page.locator('.historical-release').first()).toContainText('修复角色技能搜索、收藏挑战返回与成绩回看焦点')
    await auditLayout(page)
    await page.keyboard.press('Escape')
