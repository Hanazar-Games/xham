...
for (const [width, height] of [[320, 568], [390, 844], [540, 720], [768, 1024], [1024, 768], [1440, 1000], [844, 390]]) {
  test(`layout and dialogs at ${width}×${height}`, async ({ page }) => {
    ...
    await expect(page.locator('.historical-release')).toHaveCount(17)
    await page.locator('.historical-release summary').first().click()
    await expect(page.locator('.historical-release').first()).toContainText('v0.13.0')
    await expect(page.locator('.historical-release').first()).toContainText('新增《钢之炼金术师 Brotherhood》50 道配图题')
    await auditLayout(page)
    await page.keyboard.press('Escape')
...
test('discovery filters, sorting, favorites and reset on refresh', async ({ page }) => {
  ...
  for (const [query, id] of [['re0', 'rezero'], ['ONE PIECE', 'one-piece'], ['NARUTO', 'naruto'], ['鬼灭', 'demon-slayer']]) {
    ...
    await expect(page.locator('.quiz-card')).toHaveCount(9)
    await expect(page.locator('.quiz-card h3')).toHaveText(quizzes.filter((quiz) => quiz.series === id || quiz.series === 'crossover').map((quiz) => quiz.title))
  }
  ...
  await expect(page.locator('.quiz-card')).toHaveCount(36)
