...
test('random challenges respect the series, difficulty and saved library on mobile', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: '搜索 Quiz' }).fill(title)
...
test('the anime center is the home and every series has three difficulty levels', async ({ page }) => {
  await page.goto('/')
  await expect(page.locator('h1')).toContainText('2 dimention')
  await expect(page.locator('.page-intro')).toContainText('hanazar 的二次元中心')
  await expect(page.locator('.anime-series-card')).toHaveCount(54)
  ...
  for (const series of animeSeries) {
    ...
    await page.getByRole('button', { name: '中等', exact: true }).click()
    await expect(page.locator('.quiz-card')).toHaveCount(1)
    await expect(page.locator('.quiz-card .difficulty')).toHaveText(['中等'])
    await page.getByRole('button', { name: '简单', exact: true }).click()
...
test('a series challenge returns to its filtered library and every navigation stays in the anime center', async ({ page }) => {
  ...
  await expect(page.locator('.timer strong')).toHaveText('25')
