...
test('Haikyuu opens season one with fifty questions and later adaptations excluded', async ({ page }) => {
  ...
  await expect(page.locator('.catalog-item[data-number="37"]')).toContainText('待制作')
...
test('catalog preserves all ten batches and only opens existing question banks', async ({ page }) => {
  ...
  await expect(dialog.locator('.catalog-summary')).toContainText('37 / 200')
  await expect(dialog.locator('.catalog-summary')).toContainText('1,850 / 10,000')
