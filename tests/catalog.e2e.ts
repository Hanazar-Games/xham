...
test('Seven Deadly Sins opens its first-season bank with sequel exclusions', async ({ page }) => {
  ...
  await expect(page.locator('.catalog-item[data-number="35"]')).toContainText('待制作')
...
test('catalog preserves all ten batches and only opens existing question banks', async ({ page }) => {
  ...
  await expect(dialog.locator('.catalog-summary')).toContainText('35 / 200')
  await expect(dialog.locator('.catalog-summary')).toContainText('1,750 / 10,000')
