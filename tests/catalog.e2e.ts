...
test('KonoSuba opens its first-season bank with skill and adaptation scope', async ({ page }) => {
  ...
  await expect(page.locator('.catalog-item[data-number="39"]')).toContainText('待制作')
...
test('catalog preserves all ten batches and only opens existing question banks', async ({ page }) => {
  ...
  await expect(dialog.locator('.catalog-summary')).toContainText('39 / 200')
  await expect(dialog.locator('.catalog-summary')).toContainText('1,950 / 10,000')
