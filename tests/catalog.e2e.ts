...
test('Angel Beats opens its TV bank with special and game exclusions', async ({ page }) => {
  ...
  await expect(page.locator('.catalog-item[data-number="36"]')).toContainText('待制作')
...
test('catalog preserves all ten batches and only opens existing question banks', async ({ page }) => {
  ...
  await expect(dialog.locator('.catalog-summary')).toContainText('36 / 200')
  await expect(dialog.locator('.catalog-summary')).toContainText('1,800 / 10,000')
