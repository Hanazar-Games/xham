...
test('Parasyte opens a complete TV bank with three levels and adaptation boundaries', async ({ page }) => {
  ...
  await expect(page.locator('.catalog-item[data-number="44"]')).toContainText('待制作')
...
test('catalog preserves all ten batches and only opens existing question banks', async ({ page }) => {
  ...
  await expect(dialog.locator('.catalog-summary')).toContainText('44 / 200')
  await expect(dialog.locator('.catalog-summary')).toContainText('2,200 / 10,000')
