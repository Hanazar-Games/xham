...
test('Akame opens fifty illustrated TV questions with adaptation boundaries', async ({ page }) => {
  ...
  await expect(page.locator('.catalog-item[data-number="32"]')).toContainText('待制作')
...
test('catalog preserves all ten batches and only opens existing question banks', async ({ page }) => {
  ...
  await expect(dialog.locator('.catalog-summary')).toContainText('32 / 200')
  await expect(dialog.locator('.catalog-summary')).toContainText('1,600 / 10,000')
