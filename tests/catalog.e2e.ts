...
test('Promised Neverland opens its first-season fifty-question bank', async ({ page }) => {
  ...
  await expect(page.locator('.catalog-item[data-number="38"]')).toContainText('待制作')
...
test('catalog preserves all ten batches and only opens existing question banks', async ({ page }) => {
  ...
  await expect(dialog.locator('.catalog-summary')).toContainText('38 / 200')
  await expect(dialog.locator('.catalog-summary')).toContainText('1,900 / 10,000')
