...
test('Spirited Away opens its own film bank without counting the Ghibli collection twice', async ({ page }) => {
  ...
  await expect(page.locator('.catalog-item[data-number="48"]')).toContainText('待制作')
...
test('catalog preserves all ten batches and only opens existing question banks', async ({ page }) => {
  ...
  await expect(dialog.locator('.catalog-summary')).toContainText('48 / 200')
  await expect(dialog.locator('.catalog-summary')).toContainText('2,400 / 10,000')
